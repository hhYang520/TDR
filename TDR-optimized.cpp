...
double max_HV = 0, max_HL = 0, max_V = 0;
double ave_HV = 0, ave_HL = 0, ave_V = 0;
int good_HV = 0, good_HL = 0, good_V = 0;
...
struct SequenceQueryState {
  ...
  inline void pop_label_seq1(const int &uid, const std::vector<int> &idsSeq) {
    ...
    if (current_label != idsSeq[check_pos]) {
      need_pos = check_pos;
    }
  }
...
class ReachabilitySearcher {
 ...
 private:
  ...
  [[nodiscard]] bool forward_label_prune_all_labels(const int &gid,
    ...
    prune_forward_success++;
    return true;
  }
  //sequence1类查询(a+b+c+……)纵向分组的剪枝策略，true表示能够剪枝，false表示不能剪枝
  [[nodiscard]] bool forward_label_prune_sequence1(const int &gid, const VertexIndex &U,
                                                   const std::vector<int> &idsSeq) const {
    prune_forward++;
    //根据标签分组的纵向索引进行剪枝,只能向前看一步
    int index = gid * layer_length;
    uint8_t needed_pos = sqs.need_pos;
    const int needed_label = idsSeq[needed_pos];
    uint8_t p = lht.p[needed_label];
    uint8_t h = lht.h[needed_label];
    // 包含需要的标签，不能剪枝
    if ((U.labels_forward[index + p] & (1u << h)) != 0) {
      return false;
    }
    // !!n 将 n>0 转换为 1，n=0 转换为 0
    uint32_t mask = !!needed_pos;
    const int current_label = idsSeq[needed_pos - mask];
    p = lht.p[current_label];
    h = lht.h[current_label];
    // 包含当前标签，不能剪枝
    if ((U.labels_forward[index + p] & (1u << h)) != 0) {
      return false;
    }
  ...
  //sequence1类查询(abc……)+纵向分组的剪枝策略，true表示能够剪枝，false表示不能剪枝
  [[nodiscard]] bool forward_label_prune_sequence2(const int &gid, const VertexIndex &U,
                                                   const std::vector<int> &idsSeq) const {
    prune_forward++;
    uint8_t needed_pos = sqs.need_pos;
    const int steps = std::min(path_k, sqs.num - needed_pos);

    if (steps <= 0) {
      return false;  // 没有需要检查的标签，不能剪枝
    }

    // 对每个需要检查的步
    for (int i = 0; i < steps; ++i) {
      int temp = gid * layer_length + i * label_hash_len;
      int tl = idsSeq[needed_pos + i];
      const uint8_t p = lht.p[tl];
      const uint8_t h = lht.h[tl];
      //向前看的第i步不满足约束，则可以剪枝
      if ((U.labels_forward[temp + p] & (1u << h)) == 0) {
        prune_forward_success++;
        return true;
      }
    }
  ...
  bool search_impl(int uid, int vid,
    ...
    s[s_pos] = uid;
    bool isAdd = true;
    prune_group_vertices = 0;
    prune_group_labels = 0;
    prune_forward = 0;
    prune_group_vertices_success = 0;
    prune_group_labels_success = 0;
    prune_forward_success = 0;
    ...
    while (s_pos != -1) { //当栈为空时，遍历结束
      ...
      while (qs.gid[top_uid] < top_u.group) {
        ...
        if (qs.next[top_uid] >= source_offset[top_uid + 1]) {
          ...
          // 分组标签剪枝
          if constexpr (M == Mode::AllLabel) {
            if (forward_label_prune_all_labels(qs.gid[top_uid], top_u, bitsL)) continue;
          } else if constexpr(M == Mode::Sequence1) {
            if (forward_label_prune_sequence1(qs.gid[top_uid], top_u, idsSeq)) continue;
          } else if constexpr(M == Mode::Sequence2) {
            if (forward_label_prune_sequence2(qs.gid[top_uid], top_u, idsSeq)) continue;
          } else {
...
//  if (num_L < label_hash_len * UINT_bits) label_hash_len = (num_L + 32 - 1) / 32;
  ...
  layer_length = label_hash_len * path_k;
  ...
  while (s_pos != -1) { //当栈为空时，遍历结束
    ...
    if (finished) { //表示该顶点的所有后继顶点都已经处理完，可以出栈
      ...
      if (n == 0) {
        ...
        top_v.group = group;
        //分配内存并初始化
        top_v.labels_forward.assign(group * layer_length, 0);
        if (group > 1) {
        ...
        for (int i = source_offset[top_vid]; i < max_neighbor; i++) {
          ...
          for (int j = target_offset[i]; j < target_offset[i + 1]; ++j) {
            ...
            //第index组第一层后继标签
            top_v.labels_forward[index * layer_length + p] |= (1u << h);
          }
...
// 构建索引并计算构建时间
void index_construction(const string &result_name, const string &total_name) {
  clock_t start_time, end_time;
  start_time = clock();
      ...
      for (int i = source_offset[uid]; i < source_offset[uid + 1]; i++) {
        ...
        if (source_offset[vid] != source_offset[vid + 1]) {
          ...
          for (int j = 0; j < label_hash_len; j++) {
            ...
            //第一层循环表示对于u来说向前看的第pi步
            //第二层循环是后继顶点所有组的pi-1步标签
            for (int pi = 1; pi < path_k; ++pi) {
              for (int gi = 0; gi < v.group; ++gi) {
                int index_u = index * layer_length + pi * label_hash_len + j;
                int index_v = gi * layer_length + (pi - 1) * label_hash_len + j;
                u.labels_forward[index_u] |= v.labels_forward[index_v];
              }
            }
          }
        }
  ...
  for (auto &v: vertices) {
    ...
    if (v.group < 0)
      continue;
    index_size += (long long) sizeof(uint32_t) * v.group * layer_length;
  ...
  ofstream result_file, total_file;
  result_file.open(result_name, ios::app);
  result_file.setf(ios::fixed, ios::floatfield);
  result_file.precision(3);
  result_file << "indexTime: " << total_time << " ms";
  if (total_time >= one_hour)
    result_file << " = " << total_time / one_hour << " h" << endl;
  else if (total_time >= one_minute)
    result_file << " = " << total_time / one_minute << " min" << endl;
  else
    result_file << endl;
  result_file << "indexSpace: " << double(index_size) / (1024 * 1024) << " MB" << endl
              << endl;
  result_file.close();
...
void run_queries(const string &runtime_name, const string &total_name, const string &result_name) {
  unordered_map<int, unordered_map<int, pair<double, int>>> true_query, false_query;
  clock_t start_time, end_time;

  ofstream f(result_name, ios::out);
...
//    cout << u << " " << v << " " << q.pattern << " " << L << endl;
    ...
    if (prune_group_vertices != 0) {
      double temp = (double) prune_group_vertices_success / prune_group_vertices;
      if (temp > max_HV) max_HV = temp;
      if (temp > 0.5) {
        good_HV++;
        if (ave_HV == 0) ave_HV = temp;
        else ave_HV = (temp + ave_HV) / 2;
      }
    }
    if (prune_group_labels != 0) {
      double temp = (double) prune_group_labels_success / prune_group_labels;
      if (temp > max_HL) max_HL = temp;
      if (temp > 0.5) {
        good_HL++;
        if (ave_HL == 0) ave_HL = temp;
        else ave_HL = (temp + ave_HL) / 2;
      }
    }
    if (prune_forward != 0) {
      double temp = (double) prune_forward_success / prune_forward;
      if (temp > max_V) max_V = temp;
      if (temp > 0.5) {
        good_V++;
        if (ave_V == 0) ave_V = temp;
        else ave_V = (temp + ave_V) / 2;
      }
    }
    f << u << " " << v << " " << q.pattern << " " << q.outcome << endl;
    if (q.outcome) {
...
//      f << u << " " << v << " " << q.pattern << " " << q.outcome << endl;
  ...
  //生出输出文件
  ofstream runtime_file, total_file;
  runtime_file.open(runtime_name, ios::app);
  if (!runtime_file.is_open()) {
    cout << "Failed to create runtime runtime_file!" << endl;
    return;
  }
  runtime_file.setf(ios::fixed, ios::floatfield);
  runtime_file.precision(3);
  runtime_file << "true-queries runtime:" << endl;
  for (auto &p: true_query) {
    runtime_file << "pattern: " << p.first << ":" << endl;
    for (auto &t: p.second) {
      runtime_file << "|L|=" << t.first << ": " << t.second.first << " / " << t.second.second
                   << " = " << t.second.first / t.second.second << ";    ";
    }
    runtime_file << endl;
    cout << "pattern: " << p.first << " reachable: " << true_count[p.first] << endl;
  }
  runtime_file << "false-queries runtime:" << endl;
...
//  }
  ...
  string graphs[2] = {"Youtube", "superuser"};
//  string graphs[6] = {"citeseer", "wikitalk", "dblp", "webBerkStan", "Youtube", "superuser"};
  for (auto &name: graphs) {
    ...
    string total_name = "0static_TDR_" + reach_mode;
    string runtime_name = name + "_runtime_" + reach_mode;
    string result_name = name + "_result_" + reach_mode;
    ofstream total_file, runtime_file;
    total_file.open(total_name, ios::app);
    runtime_file.open(runtime_name, ios::out);
    total_file << name << ":" << endl;
    total_file.close();

    //构建索引
    index_construction(runtime_name, total_name);
    ...
    OR_visited_count = 0;
    NOT_visited_count = 0;
    run_queries(runtime_name, total_name, result_name);
    cout << "********* Finish! *********" << endl;
    ...
    cout << endl << max_HV << " " << max_HL << " " << max_V << endl;
    cout << ave_HV << " " << ave_HL << " " << ave_V << endl;
    cout << good_HV << " " << good_HL << " " << good_V << endl;

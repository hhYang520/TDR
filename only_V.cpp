...
struct PathStateBits {
  ...
  [[nodiscard]] inline bool is_subset(const int &v, const vector<uint32_t> &tps) const {
...
class ReachabilitySearcher {
 ...
 private:
  ...
  [[nodiscard]] bool vertex_prune(const VertexIndex &U, const VertexIndex &V) const {
    ...
    // 目标顶点是叶子
    if (V.leaf_hash != -1) {
      int blk = V.leaf_hash & (vertex_hash_len - 1);
      int bit = V.leaf_hash & (UINT_bits - 1);
...
void index_construction(const string &total_name) {
        ...
        if (source_offset[vid] != source_offset[vid + 1]) {
          ...
          for (int j = 0; j < label_hash_len; j++) {
            ...
            for (int pi = 1; pi < path_k; ++pi) {
              int index_u = pi * label_hash_len + j;
              int index_v = (pi - 1) * label_hash_len + j;
  ...
  for (auto &v: vertices) {
    ...
    if (v.leaf_hash != -1)
      continue;
    index_size += (long long) sizeof(uint32_t) * layer_length;
  }
...
//  string graphs[6] = {"citeseer", "wikitalk", "dblp", "webBerkStan", "Youtube", "superuser"};
  for (auto &name: graphs) {
    ...
    string total_name = "3static_V_" + reach_mode;
    ofstream total_file;
    total_file.open(total_name, ios::app);
